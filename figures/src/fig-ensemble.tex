...
\begin{scope}[on background layer]
  \draw[draw=black!38, dashed, rounded corners=8pt, line width=0.9pt,
        fill=black!2] (-7.55, 1.86) rectangle (7.55, -2.18);
\end{scope}
\node[hdr, anchor=west] at (-7.35, 1.66)
...
\foreach \bb/\xc/\clr in {bA/\cA/clrCls, bB/\cB/clrPrompt, bC/\cC/clrPCS}{
  ...
  \foreach \dx in {-\dchip, 0, \dchip}{
    ...
    \draw[bus]  ({\xc+\dx}, {\ychip-0.44}) -- ({\xc+\dx}, \ycol);
  }
...
% Die Beschriftung steht einmal am linken Rand und gilt fuer alle drei Klammern
% — zwischen den Branches ist kein Platz, und unter jeder Klammer wuerde sie
% von den Abgaengen durchschnitten.
\node[font=\fontsize{6.2}{7.6}\selectfont\itshape, text=black!55, anchor=east,
      align=right] at ({\cA-\dchip-0.62}, {\ychip-0.34})
      {three voters\\$=$ one branch};
